"""Feature table component — shows all 16 extracted features with color indicators."""
...
def render(result: dict):
    ...
    html += add_row("SDE", features.get("sde"), green)
    ...
    html += add_row("Secondary Eclipse Depth", features.get("secondary_depth"), coral)
    
    # Counting and noise features
    ...
    html += add_row("Autocorrelation Peak", features.get("autocorr_peak"), blue)
    ...
    # Blend/crowding diagnostics
    crowding = features.get("crowding_metric", 1.0)
    crowding_color = amber if crowding is not None and crowding < 0.8 else green
    html += add_row("Crowding Metric", features.get("crowding_metric"), crowding_color)
    
    centroid = features.get("centroid_shift", 0.0)
    centroid_color = amber if centroid is not None and centroid > 0.015 else green
    html += add_row("Centroid Shift", features.get("centroid_shift"), centroid_color)
